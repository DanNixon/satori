...
async fn one() {
    ...
    let encryption = EncryptionKey::from_bytes(&[
        241, 82, 51, 229, 138, 74, 52, 87, 107, 37, 119, 249, 154, 52, 31, 64, 7, 107, 210, 255,
        31, 171, 98, 129, 70, 163, 132, 168, 166, 75, 18, 88,
    ])
    .unwrap();
    ...
    let archiver_config_file = {
        let contents = indoc::indoc!(
            r#"
            url = "s3://satori/"
            encryption_key = [241, 82, 51, 229, 138, 74, 52, 87, 107, 37, 119, 249, 154, 52, 31, 64, 7, 107, 210, 255, 31, 171, 98, 129, 70, 163, 132, 168, 166, 75, 18, 88]
            "#
        );
    ...
    // Give the system some time to process events and archive
    ...
    let s3_event = s3_bucket
        ...
        .await
        .unwrap();
    let s3_event = encryption.decrypt(s3_event.bytes().to_owned()).unwrap();
    assert_eq!(
        s3_event,
